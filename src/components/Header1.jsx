...
const Header = ({ sUBSIDIES, userNavigationWidth }) => {
  const sUBSIDIESStyle = useMemo(() => {
    return {
      width: userNavigationWidth,
    };
  }, [userNavigationWidth]);
  ...
  return (
    <header className="self-stretch bg-limegreen-100 flex flex-row items-start justify-between pt-[33px] pb-[19px] pr-[43px] pl-12 box-border max-w-full gap-[20px] text-left text-31xl text-gray-01 font-poppins mq1350:pl-6 mq1350:pr-[21px] mq1350:box-border">
      ...
      <h1
        className="m-0 w-[332px] relative text-inherit font-extrabold font-inherit inline-block shrink-0 max-w-full z-[1]"
        style={sUBSIDIESStyle}
      >
        {sUBSIDIES}
      </h1>
